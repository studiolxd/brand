 ...
 */
export function useCopyToClipboard(feedbackDuration = COPY_FEEDBACK_MS) {
  ...
  const copy = useCallback(async (value: string | (() => string)) => {
    ...
    try {
      ...
      setStatus('error');
      return { ok: false as const, text, error };
    }
  }, []);
