...
11. **Ningún componente emite un atributo `style`.** Las apps sirven `style-src 'self'` sin `'unsafe-inline'`, y el navegador descarta en silencio cualquier atributo `style` del HTML (sin violación en consola). Un valor calculado va por `data-*` + una regla por valor en la hoja (`SiteNav` `data-columns`, `ProgressBar` `data-value`), por atributos de presentación de SVG (`Chart`, `Sparkline`, `Skeleton`: `fill`, `stroke`, `width`, `height`…), o —solo para lo que se pinta en cliente, nunca para el primer render del servidor— por el CSSOM con `useCssProperties` (`src/stories/constants/css-properties.ts`). **El arrastre no es excepción**: lo que calcula un motor de arrastre (dnd-kit) aterriza en `Sortable` (v49.4.0), que lo escribe por el CSSOM en `--sortable-*` y publica el estado en `data-dragging`; el `transform` lo pone la hoja. Regla ESLint activa sobre `src/stories/**/*.tsx`; excepciones declaradas: `email/` y `og/` (medios sin CSS), y el catálogo (`*.stories.tsx`, `*.test.tsx`, `foundations/`), que no viaja en el paquete. Entró en v33.0.0 (2026-09-10).

12. **El HTML que no viene de React entra por `Prose html`** (v49.4.0), ya saneado por el consumidor: el DS no lleva sanitizador ni va a llevarlo. Un `dangerouslySetInnerHTML` suelto en un `<div>` desnudo deja ese texto fuera de la escala, del ritmo y de la medida de lectura — que es lo que `Prose` existe para dar. `html` es excluyente con `children` por tipo.
...
- **`.surface-light` — la superficie SIEMPRE clara** (v49.8.0). El contrapunto de `.surface-dark`, para el interior de un relleno **claro y universal**: el amarillo del aviso (`Alert warning`, `Banner warning`), que es el mismo color en las dos superficies. Ni `.surface-dark` (justo al revés) ni `.surface-invert` (que sobre página clara da la cara oscura) sirven ahí. No está en `DARK_SELECTORS`: sobre página clara no hace nada, porque los tokens ya están en claro; `src/tokens/surface-light.css` —generado, no editar, mismos nombres y valores que `surface-invert.css`— los devuelve a claro cuando la clase cae dentro de una superficie oscura. Mismas dos reglas que `.surface-invert`: no pinta el lienzo, y no va en el elemento que consume los tokens volteados (el `Banner` la pone en `.banner__content`, `.banner__actions` y el aspa, y copia antes su tinta en `--banner-ink`). El porqué, en Foundations → Colores § «La superficie siempre clara».
...
- **Los controles parten de `lg` en la superficie pública.** No se enumeran: un control declara su talla `lg` en tokens hermanos (`input.lg-height` junto a `input.height`), que es de donde bebe su modificador `--lg`, y el generador apunta en `.site-shell` cada token de partida a su hermano `lg-*`. De ahí salen además las semillas del punto fijo, así que lo que herede de un control (el `filter-bar.control-block-size` contra el que se centra un interruptor) sube con él. Un `size="sm"` dentro del shell sigue mandando: su modificador declara la variable en el propio elemento. Lo que sale por un portal (la lista de un `Select`, el calendario de un `DatePicker`) no hereda la superficie — mismo caso y misma respuesta que `Modal`/`Sheet`: su `container`.
- **`src/tokens/surface-public.css` es generado** (`pnpm build:tokens`, cola de `sd.config.mjs`). Vuelve a declarar bajo `.site-shell` todo token que referencie esos tokens base, porque un `var()` dentro de una custom property se sustituye en el elemento que la declara: `--alert-title-font-size: var(--text-font-size)` vive en `:root` y llega ya resuelto a 16px. No editarlo a mano; añadir la referencia en el JSON y rebuildear basta. La referencia arrastra también dentro de una fórmula (`calc({label.font-size} * …)`), no solo suelta.
