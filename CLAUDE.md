...
- Quedan sin **ningún** estilo oscuro definido — ni antes ni ahora — los componentes que nunca lo tuvieron: Toast, Radio, Avatar, Header, Accordion, Popover, Spinner, EmptyState, PasswordField, NumberInput, FileUpload (átomo), NumberBadge, MessageBubble, TypingIndicator, MessageComposer, ConversationThread/List, OrgSwitcher, UserMenu, DatePicker/DateTime/NumberInput fields. Si uno de estos se ve mal en `.surface-dark`/modo oscuro root-level, no es un bug de regresión — es diseño pendiente: hay que decidir el valor oscuro por primera vez y añadir sus tokens `surface-dark-*`, no parchear con CSS a mano en `surface.css`.
